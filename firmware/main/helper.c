...
int restart_count_get()
{
    ...
    ESP_LOGW(TAG, "restart reason: %u", reset_reason);
    if (timer) {
    ...
    if (reset_reason == RESET_REASON_CHIP_POWER_ON || reset_reason == RESET_REASON_CORE_RTC_WDT) {
        restart_count++;
        ESP_LOGD(TAG, "restart count: %u", restart_count);
    } else {
        restart_count = 1;
    }
