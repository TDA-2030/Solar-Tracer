...
// void calculateAttitude(float gyroX, float gyroY, float gyroZ, float accX, float accY, float accZ, float dt, bmi270_axis_t * axis_val)
{
    ...
    float accRoll = atan2f(imu_data->acc.y, imu_data->acc.z) * 180.0f / M_PI;
    float accPitch = atan2f(-imu_data->acc.x, sqrtf(imu_data->acc.y * imu_data->acc.y + imu_data->acc.z * imu_data->acc.z)) * 180.0f / M_PI;
